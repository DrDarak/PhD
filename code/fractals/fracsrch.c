...
#define SEARCH 2 /* switchs searching on off(0) normal(1) limited(2) */
#define VQ 0 /* switchs VQ on(1) / off(0) */
 ...
 BASIS *top)
{
	...
	/* calcualte number of bits for limited search */
	if (POLY==0 && SEARCH==2){
	   /* first search for c min/max */
           parent=image->parent_top;
	   cnt=0;
...
char *argv[];
{
	...
	compress_image(image,basis,size,grad);
