 ...
 char *loc)
{
	int cnt;
        ...
        pt=(BASIS *)calloc(size,sizeof(BASIS));
	cnt=size*sizeof(BASIS);
 
        /* check for errors */
        if (pt==NULL){
           printf("\nNULL BASIS pointer at %s\n",loc);
           exit(-1);
        }
 
        if (VERBOSE)
           printf ("mallocing %d bytes of BASIS memory at %p\n",cnt,pt);
...
(BASIS *pt)
{
	if (pt->pt!=NULL)
        ...
        if (VERBOSE)
           printf ("freeing BASIS memory at %p\n",pt);
 ...
 char *loc)
{
        ...
        pt=(ZOOM *)calloc(size,sizeof(ZOOM));
        cnt=size*sizeof(ZOOM);

        /* check for errors */
        if (pt==NULL){
           printf("\nNULL ZOOM pointer at %s\n",loc);
           exit(-1);
        }

        if (VERBOSE)
           printf ("mallocing %d bytes of ZOOM memory at %p\n",cnt,pt);
...
(ZOOM *pt)
{
        ...
        if (VERBOSE)
           printf ("freeing ZOOM memory at %p\n",pt);
