...
/*                     Bath University                          */
/*                    daveb@ee.bath.ac.uk                       */
/*	using old method of splitting using a thrshold 		*/
/*			reworked 28/2/97			*/	
/*##############################################################*/
 ...
 int budget)
{
        ...
        /* now quadtree image */
	for (j=4000;bits<budget && j>10 ;j=j*9/10){

	printf("%d\n",j);
	t_hold=j*basis->size*basis->size*4;
	for (size=basis->size;size>2;size/=2){
	printf("%d\t%f\n",size,t_hold);
            block=image->tree_top;
	    t_hold/=4;
            while (bits<budget && block){

		  while (block && (block->error<t_hold || block->size!=size)){
			block=block->next;
		  }
