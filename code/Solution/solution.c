...
*/
void calculate_gradient(COEFF_PLANES *data, int n);
void smooth_gradient(COEFF_PLANES *pt);
float find_compression(COEFF_PLANES *pt, float grad);
float find_error(COEFF_PLANES *pt, float x);
float find_gradient(COEFF_PLANES *pt, float x);
/****************************************************************/
 ...
 int n)		/* number of coeffcient planes */
{
	...
	step=data->de[0]*(float)COMP_STEP;
	...
	if (myabs((xt-new_xt)) > accuracy){
	   ...
	   for (i=0;i<10;i++){
	       ...
	       best_error=(float)(256*256);
	       best_j=0;
	       for (j=0;j<n;j++) 
	           if (x_out[j]>0.0){
		      e=find_error(data+j,x_out[j]);
		      new_e=find_error(data+j,x_out[j]+residual/(float)10.0);
		      DebugF("%f\t",new_e-e);
		      diff=new_e-e;
	       ...
	       DebugF("\nbest_j=%d\t%f\n",best_j,x_out[best_j]);
	       x_out[best_j]+=residual/(float)10.0;
	   }
	}
 ...
 float grad)
{
	...
	Fatal("error - did not rturn compression \n");

 return (float)0.0; // stop strict compiler error - only reach here is Fatal() returns (!)
}

static float find_error
(COEFF_PLANES *pt,
 float x)
{
	int i;
	...
	Fatal("error - did not return error \n");

 return (float)0.0; // stop strict compiler error - only reach here is Fatal() returns (!)
}

static float find_gradient
(COEFF_PLANES *pt,
 float x)
{
	int i;
	...
	Fatal("error - did not return gradient \n");
