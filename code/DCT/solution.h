...
/* stores data on quantisation error */ 
typedef struct {
                int nitems;
                float *e,*x;
                float *de,*dx;
...
float find_gradient();
float average_gradient_of_gradient();
void solve();

void solve(data,x_out,xt,accuracy,n)
COEFF_PLANES *data;
...
int n;		/* number of coeffcient planes */
{
	...
	de=(float *)malloc(sizeof(float)*n);
...
COEFF_PLANES *pt;
{
	...
	float de0,de1,de2,scale,de_new,de_old,min_de;
        float tmp;
...
float find_compression(pt,grad)
COEFF_PLANES *pt;
float grad;
{
	int i;
