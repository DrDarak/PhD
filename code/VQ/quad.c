 ...
 int budget)
{
        ...
        while (bits<budget){
              ...
              best=block;
              while (block!=NULL){
                    if (block->error>best->error && block->size>MIN_SIZE)
                       best=block;
                    block=block->next;
              }
...
#ifdef PART_VQ
        first=loadVQ("list.vq");
	first=partitionImage(image,first);
        first=saveVQ("list.vq",first);
#endif
